...
from blockwise_power_tb_train import (
    ...
    unwrap_model,
    vargrad_tb_loss,
    vargrad_tb_loss_streaming_backward,
)
        ...
        for start in batch_iter:
            ...
            for micro_start in range(0, len(batch_indices), args.micro_batch_size):
                ...
                if args.stream_score_backward:
                    debug_log(f"[block {block_idx}] step {step_id} streaming loss/backward begin", rank=rank)
                    loss, logp_theta, logp_ref = vargrad_tb_loss_streaming_backward(
                        model,
                        tokenizer,
                        sequences,
                        prompt_lens,
                        ...
                        args.score_micro_batch_size or len(micro_df),
                        micro_sequences / total_sequences,
                    )
                    debug_log(f"[block {block_idx}] step {step_id} streaming loss/backward end", rank=rank)
                else:
